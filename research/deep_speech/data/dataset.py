...
class DeepSpeechDataset(object):
  ...
  def _preprocess_data(self, file_path):
    ...
    features = pool.map(self._preprocess_audio, [line[0] for line in lines])
    labels = pool.map(self._preprocess_transcript, [line[2] for line in lines])
    pool.terminate()
  ...
  def _normalize_audio_tensor(self, audio_tensor):
    ...
    mean = np.mean(audio_tensor, axis=0)
    var = np.var(audio_tensor, axis=0)
    normalized = (audio_tensor - mean) / (np.sqrt(var) + 1e-6)
    return normalized
