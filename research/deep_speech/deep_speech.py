...
def evaluate_model(estimator, speech_labels, entries, input_fn_eval):
  ...
  y_preds = [pred["probabilities"] for pred in predictions]
...
def model_fn(features, labels, mode, params):
  ...
  ctc_input_length = deep_speech_model.compute_length_after_conv(
      [tf.shape(features)[1], tf.shape(probs)[1], input_length])
  # Compute CTC loss
  loss = tf.reduce_mean(deep_speech_model.ctc_loss(
