...
################################################################################
def batch_norm_relu(inputs, training, channel_format):
  """Performs a batch normalization followed by a ReLU."""
  # We set fused=True for a significant performance boost. See
  # https://www.tensorflow.org/performance/performance_guide#common_fused_ops
  inputs = tf.layers.batch_normalization(
      inputs=inputs, axis=1 if channel_format == 'channels_first' else 3,
...
def fixed_padding(inputs, kernel_size, channel_format):
  """Pads the input along the spatial dimensions independently of input size.

  Args:
    inputs: A tensor of size [batch, channels, height_in, width_in] or
      [batch, height_in, width_in, channels] depending on channel_format.
    kernel_size: The kernel to be used in the conv2d or max_pool2d operation.
                 Should be a positive integer.
    channel_format: The input format ('channels_last' or 'channels_first').
...
def conv2d_fixed_padding(inputs, filters, kernel_size, strides, channel_format):
  """Strided 2-D convolution with explicit padding."""
  # The padding is consistent and is based only on `kernel_size`, not on the
  # dimensions of `inputs` (as opposed to using `tf.layers.conv2d` alone).
  if strides > 1:
    inputs = fixed_padding(inputs, kernel_size, channel_format)
...
def building_block(inputs, filters, training, projection_shortcut, strides,
  ...
  """
  shortcut = inputs
  inputs = batch_norm_relu(inputs, training, channel_format)
...
def block_layer(inputs, filters, block_fn, blocks, strides, training, name,
  ...
  for _ in range(1, blocks):
    inputs = block_fn(inputs, filters, training, None, 1, channel_format)
...
class Model(object):
  ...
  def __init__(self, resnet_size, num_classes, num_filters, kernel_size,
               ...
               block_strides, final_size, channel_format=None):
    ...
    Args:
      ...
      channel_format: Input format ('channels_last', 'channels_first', or None).
  ...
  def __call__(self, inputs, training):
    ...
    inputs = conv2d_fixed_padding(
        ...
        strides=self.conv_stride, channel_format=self.channel_format)
    ...
    inputs = batch_norm_relu(inputs, training, self.channel_format)
...
def resnet_model_fn(features, labels, mode, model_class,
                    ...
                    channel_format, loss_filter_fn=None, multi_gpu=False):
  ...
  model = model_class(resnet_size, channel_format)
...
def resnet_main(flags, model_function, input_function):
  ...
  classifier = tf.estimator.Estimator(
      ...
      params={
          ...
          'channel_format': flags.channel_format,
...
class NewResNetArgParser(official.utils.arg_parsers.BaseParser):
  ...
  def __init__(self, resnet_size_choices=None):
    ...
    super(NewResNetArgParser, self).__init__()

    self._add_location_args(data=True, model=True)
    self._add_performance_args(num_parallel_calls=True, channel_format=True)
  ...
  def _add_supervised_args(self, **kwargs):
    super()._add_supervised_args(**kwargs)
    self._shortcut_add(str, 'resnet_size', 's', default=50,
                 choices=self.resnet_size_choices,
                 help_text='The size of the ResNet model to use.')


if __name__ == "__main__":
  parser = NewResNetArgParser()
